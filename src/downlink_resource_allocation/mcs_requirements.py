...
class McsRequirementModel:
    ...
    def mi_qam_awgn(self, M, rho_linear, n_samples=1500, rng=None, constellation=None):
        """Estimate MI (bits/symbol) for M-QAM in AWGN at a given SINR."""
        if rho_linear <= 0.0:
            return 0.0
    ...
    def build_mi_curve(self, M, n_samples=1500):
        """Build and cache MI-vs-SINR curve used for MCS requirement lookup."""
        key = (
            int(M),
    ...
    def invert_mi_for_se(self, M, se_target, n_samples=1500):
        """Invert MI curve to find required SINR for a target spectral efficiency."""
        if se_target <= 0.0:
            return 0.0
    ...
    def required_sinr_from_mcs(self, mcs_row, l_impl_db, n_samples=1500):
        """Compute SINR requirement for one MCS row including implementation loss."""
        qm = int(mcs_row["qm"])
        M = 2 ** qm
