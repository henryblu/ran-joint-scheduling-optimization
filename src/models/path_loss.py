"""Shared large-scale path-loss and channel-gain helpers."""
...
class PathLossModel:
    ...
    def effective_path_loss_db(self, distance_m):
        """Return shadow-margin-adjusted path loss for the configured model."""

        if self.model == "fspl":
            base_pl_db = self.free_space_path_loss_db(self.fc_hz, distance_m)
        elif self.model == "umi_sc_los":
            base_pl_db = self.umi_sc_los_path_loss_db(
                self.fc_hz,
                distance_m,
                self.h_bs_m,
                self.h_ut_m,
            ...
            base_pl_db = self.umi_sc_nlos_path_loss_db(
                self.fc_hz,
                distance_m,
                self.h_bs_m,
                self.h_ut_m,
    ...
    def channel_gain_linear(self, path_loss_db):
        """Convert antenna gains and path loss into linear channel gain."""
