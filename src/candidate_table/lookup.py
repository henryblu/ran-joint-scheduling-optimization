...
from .artifact import load_or_build_candidate_table
from .logging import emit_lookup_console_log
...
def lookup_user_parameter_space(request):
    """Return one user's single-slot rows from the precomputed distance-bin table."""
    ...
    user_space = (
        ...
        .loc[lambda table: table["bits_per_slot"].astype(float) > 0.0, BATCH_USER_PARAMETER_SPACE_COLUMNS]
        .copy()
        .reset_index(drop=True)
    )
    emit_lookup_console_log(
...
def _distance_binned_candidate_table():
    ...
    return load_or_build_candidate_table()
