...
- **Owns:** Shared configuration values, the scheduler user-table column contract, radio scenario presets, default file paths, day-cycle session-generation presets, the default `SyntheticSessionGenerationConfig`, PA catalog loading, PA helper behavior used by higher layers, and config-owned scheduler solver policy such as K-MILP tolerances and limits.
...
- **Does not own:** Candidate-table generation or lookup, finite-buffer demand/test-case generation, day-run orchestration, campaign chunking, post-run analysis, notebook presentation, or CLI parsing.
...
- **Consumes:** Shared defaults from `configs`, shared run contracts from `models`, demand generation from `day_cycle_simulation`, trusted single-user batch artifacts from `candidate_table`, and the shared scheduler dispatcher in `schedulers`.
...
1. `src/configs` defines the shared radio assumptions, the scheduler user-table column contract, day-cycle presets, PA source paths, PA catalog behavior, and scheduler solver policy.
...
8. `src/day_run` orchestrates one full synthetic day by composing `day_cycle_simulation`, `candidate_table`, and `schedulers`, while `src/run_reporting.py` owns the shared console reporting used by that run layer.
